...
prompt_pure_preprompt_render() {
  ...
  # Initialize the pre and post prompt arrays.
  local -a postprompt_parts
  local -a preprompt_parts

  # Username and machine, if applicable.
  [[ -n $prompt_pure_state[username] ]] && postprompt_parts+=($prompt_pure_state[username])

  # Set the path.
  preprompt_parts+=("%(12V.%F{prompt_pure_colors[white]}(%12v) %f.)")
  preprompt_parts+=('%(?.%F{$prompt_pure_colors[prompt:success]}.%F{$prompt_pure_colors[prompt:error]})[%c]%f ')

  # Git branch and dirty status info.
  typeset -gA prompt_pure_vcs_info
  if [[ -n $prompt_pure_vcs_info[branch] ]]; then
    postprompt_parts+=("%F{$git_color}"'${prompt_pure_vcs_info[branch]}'"%F{$git_dirty_color}"' ${prompt_pure_git_dirty}%f')
  fi
  # Git action (for example, merge).
  if [[ -n $prompt_pure_vcs_info[action] ]]; then
    postprompt_parts+=("%F{$prompt_pure_colors[git:action]}"'$prompt_pure_vcs_info[action]%f')
  fi
  # Git pull/push arrows.
  if [[ -n $prompt_pure_git_arrows ]]; then
    postprompt_parts+=('%F{$prompt_pure_colors[git:arrow]}${prompt_pure_git_arrows}%f')
  fi
  # Git stash symbol (if opted in).
  if [[ -n $prompt_pure_git_stash ]]; then
    postprompt_parts+=('%F{$prompt_pure_colors[git:stash]}${PURE_GIT_STASH_SYMBOL:-≡}%f')
  fi

  PROMPT="${(j..)preprompt_parts}"
  RPROMPT=${(j. .)postprompt_parts}
